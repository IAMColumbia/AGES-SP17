...
public class Board : MonoBehaviour
{
    ...
    public void BuildInGameLevel()
    {
        ...
        for(int row = 0; row < boardTiles.Count; row++)
        {
            for(int column = 0; column < boardTiles[row].Count; column++)
            {
                ...
                currentTile.Init(new Vector2(row, column));
                currentTile.GameInit();
            }
        }
    }
    ...
    //TODO: CHANGE THESE FROM FOREACH TO FOR LOOPS (IT'LL BE A LOT EASIER TO READ!)
    ...
    public Player GetPlayerFromBoard()
    {
        for (int row = 0; row < boardTiles.Count; row++)
        {
            for (int column = 0; column < boardTiles[row].Count; column++)
            {
                ...
                if (currentTile.ObjectOnTile && currentTile.ObjectOnTile.GetComponent<Player>())
                {
                    return currentTile.ObjectOnTile.GetComponent<Player>();
                }
            }
        }
        ...
        if (StateManager.playerState != StateManager.PlayerState.DEAD)
        {
            ...
            if (target.GetComponent<IMovable>() != null)
            {
                    ...
                    newTileIndex.y < boardTiles[(int)newTileIndex.x].Count && newTileIndex.y >= 0)
                {
                    ...
                    ////There will be an out of index error if this action could not be completed
                    //try { targetTile = boardTiles[(int)newTileIndex.x][(int)newTileIndex.y]; }
                    //catch
                    //{
                    ...
                    //if the targetTile exists AND it can be walked on, move the piece to that tile
                    //if (targetTile != null)
                    //{
    ...
    {
        LevelManager.SaveLevel("Hello_World", boardTiles);
    }
    ...
    public void LoadBoard()
    {
        ...
        //load the new one
        boardTiles = LevelManager.LoadLevel("Hello_World");
        BuildInGameLevel();
    }
}
