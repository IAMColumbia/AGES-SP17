...
public class GameManager : MonoBehaviour
    {
        ...
        [SerializeField]
        public PlayerManager[] m_Planes;
        [SerializeField]
        public EnemyManager[] e_Planes;
    ...
    //The scripts are aligned with player manager not tank manager...I think.
        ...
        private void SpawnAllTanks()
        {
            ...
            {
                m_Planes[i].m_Instance = Instantiate(m_TankPrefab, m_Planes[i].m_SpawnPoint.position, m_Planes[i].m_SpawnPoint.rotation) as GameObject;
                m_Planes[i].m_PlayerNumber = i + 1;
                m_Planes[i].Setup();
            }
        }
        ...
        {
            Transform[] targets = new Transform[m_Planes.Length];

            for (int i = 0; i < targets.Length; i++)
            {
                targets[i] = m_Planes[i].m_Instance.transform;
            }
    ...
    public IEnumerator StartCountDown()
      ...
      //  m_RoundWinner = null; //I think this should be after...I get roundwinner
        ...
        private bool OneTankLeft()
        {
            ...
            for (int i = 0; i < m_Planes.Length; i++)
            {
                if (m_Planes[i].m_Instance.activeSelf)
                    numTanksLeft++;
            }
        ...
        private PlayerManager GetGameWinner()
        {
            for (int i = 0; i < m_Planes.Length; i++)
            {
                if (m_Planes[i].m_Wins == m_NumRoundsToWin)
                    return m_Planes[i]; //  return m_Tanks[i];
        }
            return null;
        }
        ...
        if (m_RoundWinner != null)
        {
            ...
            for (int i = 0; i < m_Planes.Length; i++)
            {
                return message += m_Planes[i].m_ColoredPlayerText + ": " + m_Planes[i].m_Wins + " WINS\n";
            }
