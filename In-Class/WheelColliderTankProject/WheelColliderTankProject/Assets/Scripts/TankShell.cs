    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        for (int i = 0; i < colliders.Length; i++)
        {
            ...
            try
            {
                ...
                TankHealth targetTankHealth = other.transform.parent.parent.gameObject.GetComponent<TankHealth>();
                if (targetTankHealth) //if targetTankHealth exists... (not null)
                {
                    Debug.Log("Tank took damage");
                    targetTankHealth.TakeDamage(damageDoneOnHit);
                }
            }
            catch { Debug.Log("TankHealth script could not be found"); }
