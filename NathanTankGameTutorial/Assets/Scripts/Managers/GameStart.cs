...
public class GameStart : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (tanks.Count >= SingletonPlayerSelect.Instance.NumberOfPlayers //if all the players are in the start trigger
            && SingletonPlayerSelect.Instance.NumberOfPlayers > 1) //AND if there is more than one player 
        {
            StartGame();
        }
    }
